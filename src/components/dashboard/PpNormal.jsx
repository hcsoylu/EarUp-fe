...
const PpEasy = () => {
  ...
  const [normalGame, setNormalGame] = useState([]);
  ...
  for (let i = 0; i < normalGame.length; i++) {
    let id = "";
    let date = "";
    let quesC = 0;
    let quesD = 0;
    ...
    for (let j = 0; j < normalGame[i].questionsInfo.length; j++) {
      id = normalGame[i]._id;
      date = normalGame[i].createdAt;
      if (normalGame[i].questionsInfo[j].trueAnswer === "C") {
        quesC++;
        if (normalGame[i].questionsInfo[j].isCorrect) {
          trueC++;
        }
      }
      if (normalGame[i].questionsInfo[j].trueAnswer === "D") {
        quesD++;
        if (normalGame[i].questionsInfo[j].isCorrect) {
          trueD++;
        }
      }
      if (normalGame[i].questionsInfo[j].trueAnswer === "E") {
        quesE++;
        if (normalGame[i].questionsInfo[j].isCorrect) {
          trueE++;
        }
      }
      if (normalGame[i].questionsInfo[j].trueAnswer === "F") {
        quesF++;
        if (normalGame[i].questionsInfo[j].isCorrect) {
          trueF++;
        }
      }
      if (normalGame[i].questionsInfo[j].trueAnswer === "G") {
        quesG++;
        if (normalGame[i].questionsInfo[j].isCorrect) {
          trueG++;
        }
    ...
    result = (trueC + trueD + trueE + trueF + trueG) * 10;
    data = {
      id,
      date,
      result,
